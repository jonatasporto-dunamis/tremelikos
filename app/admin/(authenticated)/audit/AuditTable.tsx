...
export default function AuditTable({ logs, total, page, pageSize, actors, entities, actions }: Props) {
  ...
  return (
    <div>
      {/* 11.10.1 — filtros */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 mb-3 flex flex-wrap gap-2 items-end">
        <div>
          <label htmlFor="filter-action" className="block text-xs text-gray-500 mb-1">Ação</label>
          <select id="filter-action" value={action} onChange={(e) => setAction(e.target.value)} className="px-2 py-2 border border-gray-200 rounded text-sm bg-white">
            <option value="">Todas</option>
            {actions.map((a) => <option key={a} value={a}>{a}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="filter-entity" className="block text-xs text-gray-500 mb-1">Entidade</label>
          <select id="filter-entity" value={entity} onChange={(e) => setEntity(e.target.value)} className="px-2 py-2 border border-gray-200 rounded text-sm bg-white">
            <option value="">Todas</option>
            {entities.map((e) => <option key={e} value={e}>{e}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="filter-actor" className="block text-xs text-gray-500 mb-1">Ator</label>
          <select id="filter-actor" value={actor} onChange={(e) => setActor(e.target.value)} className="px-2 py-2 border border-gray-200 rounded text-sm bg-white">
            <option value="">Todos</option>
            {actors.map((a) => <option key={a.id} value={a.id}>{a.label}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="filter-from" className="block text-xs text-gray-500 mb-1">De</label>
          <input id="filter-from" type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="px-2 py-2 border border-gray-200 rounded text-sm" />
        </div>
        <div>
          <label htmlFor="filter-to" className="block text-xs text-gray-500 mb-1">Até</label>
          <input id="filter-to" type="date" value={to} onChange={(e) => setTo(e.target.value)} className="px-2 py-2 border border-gray-200 rounded text-sm" />
        </div>
        <button type="button" onClick={clearFilters} className="text-sm px-3 py-2 border border-gray-200 rounded hover:bg-gray-50">
          Limpar
        </button>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="text-left p-2 font-medium text-gray-700">Diff</th>
            </tr>
          </thead>
...
function Pagination({ page, totalPages, action, entity, actor, from, to }: {
  ...
  return (
    <nav aria-label="Paginação" className="flex items-center justify-center gap-1 mt-3">
      {page > 1 && (
        <a href={makeUrl(page - 1)} className="px-3 py-2 border border-gray-200 rounded text-sm hover:bg-gray-50">← Anterior</a>
      )}
      <span className="px-3 py-2 text-sm text-gray-600">
        {page} / {totalPages}
      </span>
      {page < totalPages && (
        <a href={makeUrl(page + 1)} className="px-3 py-2 border border-gray-200 rounded text-sm hover:bg-gray-50">Próxima →</a>
      )}
    </nav>
  );
}
